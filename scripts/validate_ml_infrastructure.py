...
def test_ml_dependencies():
    ...
    if all_installed:
        ...
        print_info(
            "Install with: pip install scikit-learn xgboost lightgbm torch numpy pandas joblib"
        )
        return False
...
def test_ml_configuration():
    ...
    try:
        ...
        if len(ml_config.FEATURE_LOOKBACK_PERIODS) >= 5:
            print(
                f"  ✓ FEATURE_LOOKBACK_PERIODS (count: {len(ml_config.FEATURE_LOOKBACK_PERIODS)})"
            )
        else:
            print_warning(
                f"  ~ FEATURE_LOOKBACK_PERIODS (count: {len(ml_config.FEATURE_LOOKBACK_PERIODS)})"
...
def test_feature_extraction():
    ...
    try:
        ...
        if num_features >= 100:
            ...
            print_warning(
                f"Feature extraction created {num_features} features (expected 100+)"
            )
...
def test_model_instantiation():
    ...
    try:
        ...
        from backend.app.ml.models.price_predictor import (
            PricePredictionEnsemble,
            PricePredictor,
        )
        ...
        # Test PatternCNN
        try:
            pattern_cnn = PatternCNN(sequence_length=20, num_patterns=15)
