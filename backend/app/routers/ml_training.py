...
async def run_training_job(job_id: str, request: TrainingRequest):
    ...
    try:
        ...
        if request.model_type == "price":
            results = pipeline.train_price_predictor(df, request.symbol, request.interval)
        else:
...
async def get_training_status(job_id: str):
    ...
    if "completed_at" in job and job["completed_at"]:
        result["completed_at"] = job["completed_at"]. isoformat() if isinstance(job["completed_at"], datetime) else job["completed_at"]
...
async def list_jobs():
    ...
    for job_id, job in training_jobs.items():
        ...
        if "completed_at" in job and job["completed_at"]:
            job_info["completed_at"] = job["completed_at"].isoformat() if isinstance(job["completed_at"], datetime) else job["completed_at"]
