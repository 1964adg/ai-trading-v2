...
try:
    from backend.app.ml.models.pattern_cnn import PatternCNN
...
try:
    from backend.app.ml.models.price_predictor import PricePredictor
...
class MLPredictor:
    ...
    def __init__(self):
        ...
        self.technical_extractor = TechnicalFeatureExtractor(
            windows=ml_config.FEATURE_WINDOWS, lag_periods=ml_config.LAG_FEATURES
        )
            ...
            if PRICE_PREDICTOR_AVAILABLE:
                ...
                if price_predictor_path.exists():
                    self.price_predictor = PricePredictor(
                        horizons=ml_config.PREDICTION_HORIZONS
                    )
                    self.price_predictor.load(str(price_predictor_path))
                    logger.info("Price Predictor loaded successfully")
                else:
                    logger.warning(
                        f"Price Predictor not found at {price_predictor_path}"
                    )
                    self.price_predictor = None
            else:
                logger.warning(
                    "Price Predictor not available (XGBoost/LightGBM not installed)"
                )
                self.price_predictor = None

            self.models_loaded = (
                self.pattern_cnn is not None or self.price_predictor is not None
            )
    ...
    def predict_patterns(
        self, ohlcv_sequence: np.ndarray, threshold: float = 0.5
    ) -> Dict[str, float]:
        """
    ...
    def predict_prices(self, features: np.ndarray) -> Dict[int, Dict[str, float]]:
        """
    ...
    def get_model_status(self) -> Dict[str, bool]:
        ...
        """
        return {
            "pattern_cnn_loaded": self.pattern_cnn is not None
            and self.pattern_cnn.is_trained,
            "price_predictor_loaded": self.price_predictor is not None
            and self.price_predictor.is_trained,
