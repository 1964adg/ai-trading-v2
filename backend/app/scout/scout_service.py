...
class CryptoScoutService:
    ...
    SYMBOLS = [
        ...
        "LTCUSDT", "NEARUSDT", "ALGOUSDT", "FILUSDT", "APTUSDT",
    ...
    async def scan(self, min_score: float = 0) -> List[Opportunity]:
        ...
        logger.info(f"✅ Found {len(opportunities)} opportunities (score >= {min_score})")
    ...
    def _calculate_score(self, df:  pd.DataFrame, indicators: TechnicalIndicators) -> OpportunityScore:
        ...
        # Momentum score
        close = df['close']
        price_change_10 = ((close.iloc[-1] - close.iloc[-10]) / close.iloc[-10]) * 100 if len(df) >= 10 else 0
    ...
    def _generate_reason(self, df: pd.DataFrame, indicators: TechnicalIndicators, score: OpportunityScore) -> str:
    ...
    async def get_top_movers(self, interval: str = "24h", limit: int = 10) -> dict:
        ...
        # Sort by change %
        if interval == "1h":
            sorted_opps = sorted(self.opportunities, key=lambda x: x.change_1h, reverse=True)
            change_key = "change_1h"
        else:  # 24h
            sorted_opps = sorted(self.opportunities, key=lambda x: x.change_24h, reverse=True)
    ...
    async def get_market_overview(self) -> MarketOverview:
        ...
        bullish = sum(1 for opp in self.opportunities if opp.signal in [Signal.BUY, Signal.STRONG_BUY])
        bearish = sum(1 for opp in self.opportunities if opp.signal in [Signal.SELL, Signal.STRONG_SELL])
        neutral = sum(1 for opp in self.opportunities if opp.signal == Signal.NEUTRAL)
        
        avg_score = sum(opp.score.total for opp in self.opportunities) / len(self.opportunities) if self.opportunities else 0
