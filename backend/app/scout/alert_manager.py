...
class AlertManager:
    ...
    def _create_alert(
        ...
        if alert_key in self.last_alert_time:
            ...
            if time_since_last < 300:  # 5 minutes cooldown
                logger.debug(
                    f"Alert rate limited:  {alert_key} (last {time_since_last:.0f}s ago)"
                )
                return None
    ...
    def _notify_console(self, alert: Alert):
        ...
        logger.warning(
            f"{emoji} {alert.alert_type.value}:  {alert.message} | {alert.symbol} @ ${alert.price}"
        )
    ...
    def get_history(
        self, limit: Optional[int] = None, unacknowledged_only: bool = False
    ) -> AlertHistory:
        """Get alert history"""
