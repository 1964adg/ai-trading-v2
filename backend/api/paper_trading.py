...
def get_db_session():
    ...
    try:
        from backend.lib.database import get_db_session as _get_db_session
...
class OrderRequest(BaseModel):
    ...
    timeInForce: Optional[str] = Field(
        None, description="Time in force: 'GTC', 'IOC', 'FOK'"
    )
...
async def create_paper_order(
    order: OrderRequest, db: Session = Depends(get_db_session)
):
    """
        ...
        for position in positions:
            try:
                ...
                if klines:
                    ...
                    paper_trading_service.update_position_price(
                        position["id"], current_price, db=db
                    )
            except Exception as e:
...
async def close_paper_position(position_id: str, db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        if not klines:
            raise HTTPException(
                status_code=503, detail="Unable to fetch market price for closing"
            )
...
async def update_position(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update position: {str(e)}"
        )
