...
from services.advanced_orders_service import advanced_orders_service
...
class TrailingStopOrderRequest(BaseModel):
    ...
    activation_price: Optional[float] = Field(None, description="Price to activate trailing")
...
async def create_trailing_stop_order(order: TrailingStopOrderRequest):
    ...
    try:
        ...
        if not order.trail_amount and not order.trail_percent:
            raise HTTPException(status_code=400, detail="Must specify either trail_amount or trail_percent")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create Trailing Stop order: {str(e)}")
...
async def create_iceberg_order(order: IcebergOrderRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create Iceberg order: {str(e)}")
...
async def get_advanced_orders():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch advanced orders: {str(e)}")
