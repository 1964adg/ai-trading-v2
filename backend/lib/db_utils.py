    ...
    if "market" in engines:
        try:
            ...
            try:
                ...
                result = db.execute(
                    text("SELECT COUNT(*) FROM candlesticks")
                ).fetchone()
                results["market_candlesticks"] = f"{result[0]} candlesticks"

                # Check metadata table
                result = db.execute(
                    text("SELECT COUNT(*) FROM candlestick_metadata")
                ).fetchone()
            ...
            if Path(db_path).exists():
                ...
                stats[db_name] = {
                    ...
                    "fragmentation_pct": round(
                        (freelist / page_count * 100) if page_count > 0 else 0, 2
                    ),
                }
            else:
