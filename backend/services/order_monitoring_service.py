...
import asyncio
from backend.services.advanced_orders_service import (
...
class OrderMonitoringService:
    ...
    def _schedule_broadcast(self, order_type: str, order_data: dict):
        ...
        if self._websocket_manager:
            task = asyncio.create_task(
                self._broadcast_order_update(order_type, order_data)
            )
            ...
            task.add_done_callback(
                lambda t: (
                    ...
                    else print(f"[Order Monitoring] Broadcast failed: {t.exception()}")
                )
            )
    ...
    def _monitor_oco_orders(self, symbol: str, current_price: float):
        ...
        for order in advanced_orders_service.oco_orders.values():
            ...
            leg1_triggered = self._check_leg_trigger(
                order.order1, current_price, order.side
            )
            leg2_triggered = self._check_leg_trigger(
                order.order2, current_price, order.side
            )
    ...
    def _monitor_bracket_orders(self, symbol: str, current_price: float):
        ...
        for order in advanced_orders_service.bracket_orders.values():
            ...
            if not order.entry_filled:
                entry_triggered = self._check_entry_trigger(
                    order.entry_order, current_price, order.side
                )
                if entry_triggered:
            ...
            # If entry filled, check exit triggers
                ...
                # Check stop loss
                stop_triggered = self._check_stop_trigger(
                    order.stop_loss_order, current_price, order.side
                )
                if stop_triggered:
                ...
                # Check take profit
                profit_triggered = self._check_profit_trigger(
                    order.take_profit_order, current_price, order.side
                )
                if profit_triggered:
                    order.exit_filled = True
    ...
    def _check_profit_trigger(
        self, profit_order, current_price: float, side: str
    ) -> bool:
