...
class MLService:
    ...
    def _load_models(self) -> None:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    "Some ML models could not be loaded - predictions will be limited"
                )
    ...
    def _predict_patterns(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            patterns = self.predictor.predict_patterns(
                ohlcv_sequence, threshold=ml_config.MIN_CONFIDENCE_THRESHOLD
            )
    ...
    def _calculate_overall_confidence(
        self, patterns: Dict[str, float], price_predictions: Dict[int, Dict[str, float]]
    ) -> float:
    ...
    def _get_empty_insights(self, symbol: str, reason: str = "") -> Dict:
        ...
        return {
            "symbol": symbol,
            "patterns": {"detected": {}, "count": 0, "strongest": None},
            "price_predictions": {},
            "signals": [],
            "confidence": 0.0,
            "timestamp": datetime.now().isoformat(),
