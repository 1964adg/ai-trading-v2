...
# Separate base for analytics database
PatternBase = declarative_base()


class PatternCache(PatternBase):
    """
    ...
    pattern_type = Column(String, nullable=False, index=True)  # head_shoulders, double_top, etc.
...
class TradeExecutionLog(PatternBase):
    """
    ...
    # Execution result
    status = Column(String, nullable=False)  # pending, filled, partial, cancelled, failed
...
class MLModelResult(PatternBase):
    """
    ...
    is_correct = Column(Integer)  # 1 for correct, 0 for incorrect, NULL for not yet validated
    ...
    __table_args__ = (
        ...
        Index('idx_model_performance', 'model_name', 'is_correct', 'prediction_timestamp'),
        # Symbol predictions
        Index('idx_symbol_predictions', 'symbol', 'prediction_timestamp'),
    )


class AnalyticsMetrics(PatternBase):
    """
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    metric_type = Column(String, nullable=False, index=True)  # daily_pnl, pattern_success_rate, etc.
