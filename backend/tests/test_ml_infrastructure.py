...
def test_directory_structure():
    ...
    assert os.path.exists('../infrastructure/ml/model_storage'), "model_storage directory not found"
    assert os.path.exists('../infrastructure/ml/training_data'), "training_data directory not found"
...
def test_imports():
    ...
    from app.ml.utils.preprocessing import DataPreprocessor
...
def test_api_endpoints():
    ...
    assert '/ml/price-prediction/{symbol}' in routes, "Price prediction endpoint not found"
...
def test_preprocessing_utils():
    ...
    import numpy as np
    import pandas as pd
    from app.ml.utils.preprocessing import DataPreprocessor, normalize_ohlcv
    ...
    assert normalized.min() >= 0 and normalized[:, :4].max() <= 1, "OHLCV not normalized correctly"
